    ...
    try {
      ...
      if (/requires an index/i.test(msg) && /create it here/i.test(msg)) {
        ...
        setRemoteErrorCount(c => {
          ...
          if (next >= 3) {
            setOfflineMode(true);
            showNotificationMessage('Cloud errors – switched to offline mode','warning');
          }
          return next;
        });
      }
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div className="space-y-1">
                    ...
                    <p className="font-mono text-xs break-all text-gray-800 dark:text-gray-200">{import.meta.env.VITE_CLOUDINARY_CLOUD_NAME || '—'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-500 dark:text-gray-400">Unsigned Preset</p>
                    <p className="font-mono text-xs break-all text-gray-800 dark:text-gray-200">{import.meta.env.VITE_CLOUDINARY_UPLOAD_PRESET || '—'}</p>
                  </div>
                  <div className="space-y-1">
                ...
                <div className="mt-4">
                  <div className="flex flex-wrap gap-2">
                    {uploadedFiles.filter(f=>f.cloudinaryId).slice(0,5).map(f => (
                      <span key={f.id} className="inline-flex items-center px-2 py-0.5 rounded text-[10px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300" title={f.cloudinaryId}>{f.name.length>14? f.name.slice(0,12)+'…': f.name}</span>
                    ))}
                  ...
                  {activityLog.slice(0,6).map(a => (
                    <div key={a.ts + a.id} className="flex items-center justify-between text-xs p-2 rounded border border-gray-200 dark:border-gray-700">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-500 dark:text-gray-400">{a.action} • {(a.size/1024).toFixed(1)} KB • {new Date(a.ts).toLocaleTimeString()}</p>
                      </div>
